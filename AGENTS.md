...
- **Vitest collects `src/**/__tests__/**/*.test.ts` and nothing else.** Put every test file under a `__tests__` directory. A file under `src/__tests__/e2e/` runs under `pnpm test:e2e` on every pull request, and adding the `.soak.test.ts` suffix moves it to `pnpm test:soak`, which only the scheduled `Replication Soak` workflow runs.
