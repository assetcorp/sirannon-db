...
from chart_paths import figure, soak_chart
...
def soak_block(source: Source, chart_dir: str) -> str:
    ...
    for name, node in recorded:
        ...
        for row in node.get("workloads", []):
            ...
            body.extend(pair)
            if any(isinstance(soak, dict) and soak.get("windows") for soak in soaks):
                figures.append(
                    figure(
                        soak_chart(chart_dir, name, row.get("workload", "unknown")),
                        f"Achieved rate and p99 latency for both engines in every 30-second window of the "
                        f"{label} soak at {durability_label(name).lower()}.",
                    )
                )
        if not body:
            continue
    ...
    intro = (
        ...
        "appears. Each figure plots every window in order, so a periodic stall reads as a spike and a steady "
        "cost reads as a raised floor. An engine holds when it keeps at least 95% of the rate with under 1% "
        "errors and a p99 under "
