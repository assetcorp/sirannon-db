...
- Litestream proves the capture route in production: the write-ahead log is a documented format, and its frames are the changes. Sirannon captures frames in-process on the node that owns the file, which is a strictly easier position than Litestream's separate-process design. One proof is outstanding before implementation: reading the log file correctly from JavaScript while the connection pool controls checkpoint timing. That proof runs as a prototype before any ticket builds on it.
