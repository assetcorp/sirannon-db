...
- The destination handles a second filename, since both copy routes open a `-journal` file beside the target and write at most 8192 bytes to it.
...
- The VFS ships as a loadable SQLite extension that Sirannon owns, roughly two hundred lines of C. Experiments recorded in `docs/research/backup-interoperability.md` proved the route end to end on both Node drivers, with a 100 MB copy delivered and no local file created, and later runs carried it to 6.4 GB.
