...
- Sirannon builds the encryption shim into the same extension as the streaming shim, and the streaming shim encrypts inside its own write path. Stacking cannot do that job, because the streaming shim reaches the VFS beneath it only for randomness, sleep, and the clock, and it serves every read and write itself. A prototype measured a streamed copy of an encrypted database arriving in plaintext under both load orders. The two shims therefore share one page codec and one registry of keys by file path, and Sirannon registers the source's data key against the destination before a copy starts.
...
- The codec encrypts the main database, the write-ahead log, the rollback journal, and the super journal, and it refuses to open an encrypted database in any journal mode but write-ahead logging. A prototype that keyed the main file and the log alone wrote plaintext pages to a rollback journal.
- The codec encrypts into a buffer of its own and leaves SQLite's page untouched, because SQLite folds the whole in-memory page into a log frame's checksum before it hands the page to the file layer. A shim that wrote into SQLite's buffer, which is what SQLite's checksum shim does, would break recovery.
- Sirannon makes four refusals with no override. It forces temporary storage to memory, because SQLite's VFS interface gives a shim no way to tie a temporary file or a sub-journal back to its database, and the SQLite3MultipleCiphers authors document that those files cannot be encrypted. It refuses a backup whose destination carries no registered key, because Node's backup call opens the destination itself, and a prototype measured such a destination taking a readable copy while reporting success. It refuses to open an encrypted database on a runtime without the capability. It refuses a key shorter than 32 bytes.
- A backup holds ciphertext under the source's data key, which ADR 0011 fixes. The destination, the server's backup routes, and a restore never hold plaintext, and a restore needs the master key.
