...
An encrypted database has to stay a file whose page size Sirannon's restore reads without a key, and whose write-ahead log Sirannon's change-piece capture walks without one. Every encrypted file follows the layout below, which is why a later change cannot undo it, so this record fixes it. A prototype at `native/vfs-encryption-prototype/` measured every number here on Node's own SQLite, and `node native/vfs-encryption-prototype/run.mjs` reproduces them.
...
- A write-ahead log frame holds a ciphertext page and a plaintext frame header. SQLite computes each frame's checksum over the page as it holds it in memory, which is plaintext, and a VFS shim cannot change that. The checksum chain on disk therefore covers bytes that are not on disk. SQLite3MultipleCiphers computes its checksums over the ciphertext only because it edits SQLite's source, which a shim cannot do.
- Every checksum Sirannon folds is folded over plaintext, and every byte it stores or sends stays ciphertext. The change-piece capture in `core/backup/wal-log.ts` decrypts each frame's page in memory to verify the chain, and it sends the frame unchanged. The restore in `core/backup/restore-log.ts` decrypts each page before it stamps a frame's checksum, because SQLite reads that log back through the shim and folds plaintext. A restore that stamped over ciphertext would leave SQLite stopping at frame one, which drops every change piece and reports a success that returns the full copy alone. The prototype measured the capture side, where the walk accepted 0 of 15 frames as written and 15 of 15 after an in-memory decrypt.
- A full copy of an encrypted database is encrypted under the source's own data key. SQLite's backup interface reads a source page through the pager, which decrypts it, and writes it through the destination, which encrypts it again, so a copy is a fresh encryption of the same pages, and its bytes differ from the source's. The key has to be the source's, because a change piece is a run of source log frames under that key, and a restore folds those frames into the full copy inside one connection under one key. The prototype measured a destination generating a key of its own, which is the failure this rule prevents.
...
- **A fresh data key for every full copy** would give a backup chain whose base and change pieces no longer open under one key.
