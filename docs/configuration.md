...
| `metrics` | `MetricsConfig` | No | Callbacks for query timing with the rows that each statement returns or changes, for database open and close, and for each change event that Sirannon delivers to subscribers |
