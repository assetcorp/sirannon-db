...
- `start()` checks capabilities, reconciles the migration handshake, opens the live pull, and starts the push loop. It fails with `SYNC_UNSUPPORTED` against a server that announces no `sync.stream-apply`, and with `DEVICE_SYNC_NOT_ACCEPTED` against a server that keeps device sync off. Against a server that it cannot reach, `start()` resolves with the state `running` and the failure in `lastError`, and the controller retries with growing waits of up to `maxPushRetryDelayMs`. In a browser, it attempts nothing while the device reports no network and retries at once when the network returns. `pause()` keeps the cursors, `resume()` restarts the loops, and `stop()` ends them.
