...
`applyChanges(batch, resolver?)` on the database applies a batch of remote changes locally, defaulting to the LWW resolver. An implementation validates and applies the batch as follows:
...
- An implementation skips the whole of a batch whose `toSeq` is at or below the highest already-applied sequence for its source. It also skips a change already recorded in `_sirannon_applied_changes`, so applying the same batch twice changes nothing.
- An implementation groups changes by `txId` and applies each group in one transaction. It inserts a change whose target row is absent directly, and it passes a change whose row exists to the resolver (see [Conflict Resolution](03-replication.md#conflict-resolution)). It records each applied change in `_sirannon_applied_changes`.
- After a group commits, an implementation stamps the rows the applier's own triggers produced (those still holding an empty `node_id` above the pre-apply sequence) with the batch's `sourceNodeId` as origin and the highest applied `hlc`. Marking the origin as the source device is what makes the live pull suppress the echo back to that device.
...
`seq`, `table_name`, `operation`, `row_id`, and `changed_at` carry the change's sequence, table, operation, row id, and timestamp. `changed_at` is seconds since the Unix epoch, matching the change-log column of the same name. `old_data` and `new_data` hold the row images as JSON under the [tagged value encoding](02-core.md#tagged-value-encoding-normative); `new_data` is null for a delete, and `old_data` is null when the change carries no previous image. `node_id`, `tx_id`, and `hlc` carry the change's origin, transaction, and HLC timestamp, each empty when the change carries none. `tx_end` is 1 on the change marked `txEnd` and 0 otherwise.
...
On open, a device deletes staged rows at or below `device_sync_pull_seq`, applies every complete staged transaction, and keeps an incomplete tail for the resumed subscription. A device that fails to apply during this recovery still opens the subscription, so that a device below the server schema version reaches the migration handshake.
...
The manifest reports `startSeq` (the current maximum change sequence, captured before any copy) and `epoch` as decimal strings, the schema DDL in foreign-key-dependency order, per-table row counts, and the applied migration rows. The server rejects an in-memory database with `400 SNAPSHOT_UNSUPPORTED`.

The server must invoke the `beforeSnapshot` hook of [02-core.md](02-core.md#hooks) once per table the manifest lists, and once for the table a page names, before it reads any row. A hook that throws refuses the request with `403 HOOK_DENIED`. A hook that refuses one table refuses the whole snapshot.

A page returns rows for one table using keyset pagination: `afterKey` is the last key of the previous page (1 to 16 values) and `nextKey` is the last key of this page (null when `done`). `limit` is 1 to 1000, defaulting to 500, and the server further trims a page to fit an 8 MiB byte cap. `checksum` is the lowercase SHA-256 hex digest of the canonical form of `rows` (the same canonicalisation as the batch checksum); the device verifies it and fails with `SNAPSHOT_CHECKSUM_MISMATCH` on a mismatch.

The device applies the snapshot as a wipe-and-replace: it sets `device_sync_snapshot_state` to `loading`, turns foreign keys off, clears `_sirannon_staged_changes`, unwatches and drops the target tables in reverse dependency order, recreates the schema, inserts each table's pages, replaces its migration history and mirrors `user_version` (see [Migration Handshake](#migration-handshake)), sets the pull cursor to `startSeq` and `epoch`, then rewatches the tables, turns foreign keys on, and clears the state. A load interrupted before completion leaves the state set, so that a restart detects the incomplete copy and resumes the resync. While a load runs, every read and write on the database fails with `SNAPSHOT_IN_PROGRESS`; the device seeds that gate at open from the durable state marker.

Because the server captures `startSeq` before the copy and the device sets its pull cursor to it, the live pull replays every change after `startSeq` and so reconciles the writes made during the copy. The change log must therefore retain history back to `startSeq` for the copy's duration.

---

## Migration Handshake

A schema change must not go through `/changes`; a device applies migrations and the server serves them.

The schema version is the highest applied migration version (see [02-core.md](02-core.md#migrations)). A device carries `schemaVersion` on the push body and on the subscribe message, and the server treats a missing value as 0. The server gates on it:

- The server refuses a device version below its own with `MIGRATION_REQUIRED`.
- The server refuses a device version above its own with `SCHEMA_AHEAD`.
...
A device whose applied history diverges from the server's (an overlapping version whose checksum differs, or a gap in the shared history) or that hits a baseline gap must resync from a snapshot. That resync is destructive, so the controller raises a resync-required signal to the application before the wipe.
...
- **start** verifies server capabilities first and caches them; a `SYNC_UNSUPPORTED` or `DEVICE_SYNC_NOT_ACCEPTED` result aborts the start, while the controller records an indeterminate failure and continues degraded. It then reconciles the migration handshake, falling back to the local version when offline, opens the live pull, and starts the push loop. A pull that fails with `CONNECTION_ERROR` or `TIMEOUT` leaves the start complete and the state `running`; the controller records the failure and retries the pull.
- **reconnect** retries a failed pull after a wait that doubles with each consecutive failure up to the push cap, and draws each wait from the upper half of that value at random, so that devices recovering from one outage spread their retries. While the runtime reports the device offline, the controller attempts no push and no pull. When the runtime reports the network back, the controller retries both at once and resets both waits.
...
The controller must not wait for what one of these callbacks returns. A throw or a failure from any of them must not stop the controller, and neither must it stop delivery to any other callback. The controller must drop each of those failures.

The controller calls `onResyncRequired` when a resync becomes required: the server signals one, the migration handshake returns `resync-required`, or a start finds one the device still owes. It calls `onSnapshotComplete` once a snapshot load ends and the state has settled, for an automatic resync and for the application's own `downloadSnapshot()`. A failure after the wipe begins leaves the local database refusing reads and writes, so the outcome reports `databaseUsable` from the device's own load marker and `retrying` from whether the controller has scheduled another snapshot.
