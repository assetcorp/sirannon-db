...
import { DATABASE_ID, migrations, SEED_WORK_ORDERS, WORK_ORDERS_TABLE } from './schema'
...
if ((existing?.count ?? 0) === 0) {
  ...
  for (const order of SEED_WORK_ORDERS) {
    await db.execute(`INSERT INTO ${WORK_ORDERS_TABLE} (id, site, task, updated_at) VALUES (?, ?, ?, ?)`, [
      order.id,
      order.site,
      order.task,
      now,
